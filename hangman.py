...
HANGNAN_PICS = ['''
   ...
   / \  |
       ===''']
words = 'аист акула бабуин баран барсук бобр бык верблюд волк воробей ворон выдра голубь гусь жаба зебра змея индюк кит кобра коза козел койот корова кошка кролик крыса курица лама ласка лебедь лев лиса лосось лось лягушка медведь маллюск моль мул муравей мышь норка носорог обезьяна овца окунь олень орел осел панда паук питон попугай пума семга скунс собака сова тигр тритон тюлень утка форель хорек черепаха ястреб ящерица'.split()
# Эта функция возвращает случайную строку из переданного списка.
def getRanddomWord(wordList):
    wordIndex = random.randint(0, len(wordList) - 1)
    return wordList[wordIndex]

# Отображение игрового поля
def displayBoard(missedLetters, correctLetters, secretWord):
    print(HANGNAN_PICS[len(missedLetters)])
    print()
    ...
    for letter in blanks: # Показывает секретное слово с пробелами между буквами
        print(letter, end=' ')
    print()
# Возвращает букву, введённую игроком и проверяет, что игрок ввёл только одну букву и ничего больше.
...
# Эта функиция возвращает значение True, если игрок хочет сыграть заново; в противном случае возвращает False.
def playAgain():
...
missedLetters = ''
correctLetters = ''
secretWord = getRanddomWord(words)
gameIsDone = False
...
# Проверяет, превысил ли игрок лимит попыток и проиграл.
        if len(missedLetters) == len(HANGNAN_PICS) - 1:
            displayBoard(missedLetters, correctLetters, secretWord)
